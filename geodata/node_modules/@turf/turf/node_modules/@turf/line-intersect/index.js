...
var _turf_helpers = require('@turf/helpers');
var _turf_meta = require('@turf/meta');
...
var index_es6 = function (line1, line2) {
    ...
    if (debug) {
        ...
        // Color line1 as red
        _turf_meta.featureEach(line1, function (feature) {
        ...
        // Color line2 as blue
        _turf_meta.featureEach(line2, function (feature) {
    ...
    return _turf_helpers.featureCollection(results);
};
...
function intersects(line1, line2) {
    ...
    if (uA >= 0 && uA <= 1 && uB >= 0 && uB <= 1) {
        var x = x1 + (uA * (x2 - x1));
        var y = y1 + (uA * (y2 - y1));
        return _turf_helpers.point([x, y]);
    }
    return null;
}
...
function lineTree(line) {
    ...
    _turf_meta.featureEach(line, function (multiFeature) {
        ...
        _turf_meta.featureEach(multiFeature, function (feature) {
            if (feature.geometry.type !== 'LineString') {
                throw new Error('<lineTree> geometry must be a LineString');
            }
            if (multiLine) console.log(feature);
            _turf_meta.coordReduce(feature, function (previous, current, index) {
                var lineSegment = _turf_helpers.lineString([previous, current]);
...
function convertPolygonToLineStrings(polygon) {
    ...
    if (polygon.geometry === 'MultiPolygon') {
        polygon = _turf_helpers.featureCollection(flatten(polygon));
    }
    _turf_meta.featureEach(polygon, function (feature) {
        if (polygon.geometry.type !== 'Polygon') {
            throw new Error('<convertPolygonToLineStrings> geometry must be Polygon');
        }
        var coordinates = feature.geometry.coordinates;
        coordinates.forEach(function (poly) {
            results.push(_turf_helpers.lineString(poly));
        });
    });
    return _turf_helpers.featureCollection(results);
}
